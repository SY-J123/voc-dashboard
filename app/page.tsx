...
const problemTypes = [
  { l1: "UI·안내", l2: "화면 혼란, 피드백 부재, 안내 부족", meaning: "보면 알 수 있어야 할 것이 보이지 않거나 헷갈리는 경우" },
  { l1: "기능·성능", l2: "기능 오류, 성능·안정성, 업데이트 회귀", meaning: "동작이 되지 않거나, 느리거나, 앱이 꺼지는 경우" },
  { l1: "인증·보안", l2: "인증 실패, 절차·빈도 과다", meaning: "보안 절차가 사용성을 해치는 경우" },
  { l1: "정책·CS", l2: "광고·알림 과다, 수수료·한도, CS 불만, 기능 요청, 기능 제약", meaning: "정책 결정에 대한 불만·요구" },
  { l1: "긍정", l2: "만족·칭찬", meaning: "구체적인 단서 없이 표현된 일반적인 칭찬" },
  { l1: "_미분류", l2: "_근거부족·다중가설", meaning: "단서가 부족하거나 가설이 여러 갈래라 분류를 보류한 경우" },
];
...
const TOC = [
  ...
  { href: "#analysis", label: "분석 방식" },
  { href: "#classification", label: "분류 체계" },
  { href: "#preprocessing", label: "데이터 전처리" },
  { href: "#ground-truth", label: "정답 데이터 구축" },
  { href: "#accuracy", label: "정확도 검증·개선" },
  ...
  return (
    <div className="mx-auto max-w-[1100px]">
      <div className="flex gap-12 items-start">
        ...
        {/* 본문 */}
        <article className="flex-1 min-w-0 space-y-10 break-keep">
          ...
          {/* 분석 방식 */}
          <Section id="analysis" title="분석 방식">
          ...
          {/* 분류 체계 */}
          <Section id="classification" title="분류 체계">
          ...
          {/* 데이터 전처리 */}
          <Section id="preprocessing" title="데이터 전처리">
          ...
          {/* 정답 데이터 구축 */}
          <Section id="ground-truth" title="정답 데이터 구축">
          ...
          {/* 정확도 검증·개선 */}
          <Section id="accuracy" title="정확도 검증·개선">
            <p>
          ...
          {/* 한계 */}
          <Section id="limitations" title="한계">
            <div className="overflow-x-auto">
              <table className="text-sm border-collapse border border-neutral-200 bg-white">
                <thead className="bg-neutral-50">
                  <tr>
                    <th className="w-40 border border-neutral-200 px-3 py-2 text-left font-medium">항목</th>
                    <th className="border border-neutral-200 px-3 py-2 text-left font-medium">내용</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td className="border border-neutral-200 px-3 py-2 font-medium align-top">스냅샷 분석</td>
                    <td className="border border-neutral-200 px-3 py-2 align-top">특정 기간 데이터의 결과로, 최신 트렌드나 앱 업데이트 이후 변화를 반영하지 않는다.</td>
                  </tr>
                  <tr>
                    <td className="border border-neutral-200 px-3 py-2 font-medium align-top">Google Play 단독</td>
                    <td className="border border-neutral-200 px-3 py-2 align-top">App Store 리뷰는 포함하지 않아 iOS 사용자의 의견이 빠져 있다.</td>
                  </tr>
                  <tr>
                    <td className="border border-neutral-200 px-3 py-2 font-medium align-top">분류 정확도</td>
                    <td className="border border-neutral-200 px-3 py-2 align-top">API 호출 비용과 수동 라벨링 시간 제약으로 검증 표본을 150건으로 제한했다. L2 기준 82% 수준으로, 약 5건 중 1건은 틀릴 수 있다. 실무 적용 시에는 더 많은 샘플을 수동 라벨링해 정확도를 높이는 것을 권장한다.</td>
                  </tr>
                  <tr>
                    <td className="border border-neutral-200 px-3 py-2 font-medium align-top">분류 항목 커버리지</td>
                    <td className="border border-neutral-200 px-3 py-2 align-top">기존 항목으로 분류하기 어려운 리뷰는 _미분류로 처리된다. 새로운 유형의 불만이 증가하면 기준을 재검토해야 한다.</td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        {/* 우측 목차 */}
        <nav className="hidden xl:block w-40 shrink-0 sticky top-24 space-y-1">
          <p className="text-[10px] font-semibold uppercase tracking-widest text-neutral-400 mb-3">목차</p>
          {TOC.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="block text-xs text-neutral-500 hover:text-neutral-900 py-0.5 transition-colors"
            >
              {item.label}
            </a>
          ))}
